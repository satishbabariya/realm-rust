# Parked: `upstream/src/realm/util/enum.cpp`

Queue position #4 (depth 0). Parked 2026-08-08.
...
```
util/enum.cpp.o: 0/15 symbols linked into build/oracle/trace_runner
```

Its consumers are sync-only. `util/enum.hpp` has no includer anywhere under
`upstream/src/` outside the unit itself.
