# Parked: `upstream/src/realm/util/bson/regular_expression.cpp`

Queue position #7 (depth 0). Parked 2026-08-08.
...
```
util/bson/regular_expression.cpp.o: 0/14 symbols linked into build/oracle/trace_runner
```

Worth noting as the case that source-grepping gets wrong: it has 20 consumers outside
`sync/`, all of them in `util/bson`, which is itself unreachable with
`REALM_APP_SERVICES=OFF`. Transitive deadness is only visible at the link, which is why
the symbol-intersection check is the one to trust.
