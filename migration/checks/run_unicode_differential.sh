#!/usr/bin/env bash
# Differential check for unicode.cpp -- the only real evidence for this unit.
#
...
# unicode.cpp.o's undefined set reaches Node, Allocator and terminate, so
...
WORK="$ROOT/build/out/unicode-diff"
...
SRC="$ROOT/migration/checks/unicode_differential.cpp"

echo "==> building driver against C++ unicode"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$SRC" "$REALM_A" "${LIBS[@]}"

echo "==> building driver against Rust unicode (staticlib first)"
...
# Guard. unicode.cpp has no unique file-static and its ZT* are coalesced, so fall
...
repl=$(awk '/realm8case_mapENS_10StringDataEb$/{print $1; exit}' "$WORK/syms_rust.txt")
if [ -z "$repl" ]; then echo "FAIL: case_map missing from driver_rust." >&2; exit 1; fi
d=$(( 0x$probe > 0x$repl ? 0x$probe - 0x$repl : 0x$repl - 0x$probe ))
if [ "$d" -gt 65536 ]; then
  echo "FAIL: case_map is $d bytes from the crate probe; with codegen-units=1" >&2
...
if diff -q "$WORK/out_cxx.txt" "$WORK/out_rust.txt" >/dev/null; then
  echo "PASS: C++ and Rust agree on all $lines probe lines (exhaustive byte sweeps, case folding, LIKE)."
else
