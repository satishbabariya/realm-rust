#!/usr/bin/env bash
# Differential check for object_id.cpp -- the only real evidence for this unit.
#
...
# object_id.cpp.o's undefined set reaches Node, Allocator and terminate, so
...
WORK="$ROOT/build/out/object-id-diff"
...
SRC="$ROOT/migration/checks/object_id_differential.cpp"

echo "==> building driver against C++ object_id"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$SRC" "$REALM_A" "${LIBS[@]}"

echo "==> building driver against Rust object_id (staticlib first)"
...
# Guard. object_id.cpp has an ideal fingerprint: a static initialiser,
# __GLOBAL__sub_I_object_id.cpp, emitted only by that translation unit. Its presence
# means the C++ object was extracted; its absence means the Rust definitions won.
if grep -q "GLOBAL__sub_I_object_id" "$WORK/syms_rust.txt" 2>/dev/null; then
  echo "FAIL: driver_rust contains the C++ static initialiser for object_id.cpp." >&2
  echo "      The C++ object was extracted; this comparison would be C++ vs C++." >&2
  exit 1
fi
if ! grep -q "GLOBAL__sub_I_object_id" "$WORK/syms_cxx.txt" 2>/dev/null; then
  echo "FAIL: driver_cxx lacks the C++ static initialiser -- wrong object linked." >&2
  exit 1
fi
...
if diff -q "$WORK/out_cxx.txt" "$WORK/out_rust.txt" >/dev/null; then
  echo "PASS: C++ and Rust agree on all $lines probe lines (byte order, string round-trips, hashes, sequence deltas)."
else
