#!/usr/bin/env bash
# Differential check for status.cpp -- the only real evidence for this unit.
#
...
# status.cpp.o's undefined set reaches Node, Allocator and terminate, so
...
WORK="$ROOT/build/out/status-diff"
...
SRC="$ROOT/migration/checks/status_differential.cpp"

echo "==> building driver against C++ status"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$SRC" "$REALM_A" "${LIBS[@]}"

echo "==> building driver against Rust status (staticlib first)"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_rust" "$SRC" "$RUST_LIB" "$REALM_A" "${LIBS[@]}"

# Guard. status.cpp has no unique file-static to fingerprint -- its only static,
# `Status::reason() const::empty`, is a weak inline-function local with two definers in
# librealm.a, so it survives in both builds. Fall back to the crate probe, plus the
# stronger fact that the four symbols this unit exports are STRONG (T): if the C++
# object had been extracted alongside the Rust definitions the link would have failed
# with duplicate symbols rather than silently preferring one.
...
create=$(awk '/Status9ErrorInfo6create/{print $1; exit}' "$WORK/syms_rust.txt")
if [ -z "$create" ]; then echo "FAIL: ErrorInfo::create missing from driver_rust." >&2; exit 1; fi
d=$(( 0x$probe > 0x$create ? 0x$probe - 0x$create : 0x$create - 0x$probe ))
if [ "$d" -gt 65536 ]; then
  echo "FAIL: ErrorInfo::create is $d bytes from the crate probe; with codegen-units=1" >&2
...
if diff -q "$WORK/out_cxx.txt" "$WORK/out_rust.txt" >/dev/null; then
  echo "PASS: C++ and Rust agree on all $lines probe lines (strings across the SSO boundary, refcounts, streaming)."
else
