#!/usr/bin/env bash
# Differential check for string_data.cpp — the evidence `make diff-test` cannot give.
#
# murmur2_or_cityhash feeds string-index keys, so a divergent hash writes different
# bytes into a .realm. But no trace in harness/traces/ builds a string index, so the
# byte-identity gate never evaluates these functions. This script builds one driver
# twice — once against the oracle's C++ object, once against the Rust staticlib — and
...
WORK="$ROOT/build/out/string-data-diff"

CXX_OBJ="$ORACLE_DIR/realm-core/src/realm/CMakeFiles/Storage.dir/string_data.cpp.o"
HOST_ARCH="$(uname -m)"
case "$HOST_ARCH" in
...
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$ROOT/migration/checks/string_data_differential.cpp" "$CXX_OBJ"

echo "==> building driver against the Rust staticlib"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_rust" "$ROOT/migration/checks/string_data_differential.cpp" "$RUST_LIB" \
    -framework Foundation -framework Security -lz -lcompression

# string_data.cpp inlines everything into its five exported functions, so unlike
...
for sym in _ZN5realm10murmur2_32EPKhm _ZN5realm11cityhash_64EPKhm _ZN5realm19murmur2_or_cityhashEPKhm; do
