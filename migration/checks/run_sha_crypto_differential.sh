#!/usr/bin/env bash
# Differential check for util/sha_crypto.cpp.
#
# Both stacks ultimately call CommonCrypto, so agreeing on digest values is close to
# guaranteed. This check exists for the three things that are NOT guaranteed:
#
#   1. the Span ABI — fixed-extent Span<T,N> is pointer-only, dynamic Span<T> is
#      {ptr,size}; modelling either wrongly shifts every later argument register;
#   2. the CommonCrypto algorithm ordinals (kCCHmacAlgSHA224 = 5, not 4);
...
WORK="$ROOT/build/out/sha-crypto-diff"

CXX_OBJ="$ORACLE_DIR/realm-core/src/realm/CMakeFiles/Storage.dir/util/sha_crypto.cpp.o"
HOST_ARCH="$(uname -m)"
case "$HOST_ARCH" in
...
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$ROOT/migration/checks/sha_crypto_differential.cpp" "$CXX_OBJ"

echo "==> building driver against the Rust staticlib"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_rust" "$ROOT/migration/checks/sha_crypto_differential.cpp" "$RUST_LIB" \
    -framework Foundation -framework Security -lz -lcompression

# util/sha_crypto.cpp inlines everything into its five exported functions, so unlike
...
for sym in _ZN5realm4util4sha1EPKcmPh _ZN5realm4util6sha256EPKcmPh; do
