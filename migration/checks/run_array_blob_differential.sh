#!/usr/bin/env bash
# Differential check for array_blob.cpp -- the only real evidence for this unit.
#
...
# array_blob.cpp.o's undefined set reaches Node, Allocator and terminate, so
...
WORK="$ROOT/build/out/array-blob-diff"
...
SRC="$ROOT/migration/checks/array_blob_differential.cpp"

echo "==> building driver against C++ array_blob"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$SRC" "$REALM_A" "${LIBS[@]}"

echo "==> building driver against Rust array_blob (staticlib first)"
...
# Guard. array_blob.cpp has no unique file-static and its ZT* are coalesced, so fall
...
repl=$(awk '/ArrayBlob7replaceEmmPKcmb$/{print $1; exit}' "$WORK/syms_rust.txt")
if [ -z "$repl" ]; then echo "FAIL: ArrayBlob::replace missing from driver_rust." >&2; exit 1; fi
d=$(( 0x$probe > 0x$repl ? 0x$probe - 0x$repl : 0x$repl - 0x$probe ))
if [ "$d" -gt 65536 ]; then
  echo "FAIL: ArrayBlob::replace is $d bytes from the crate probe; with codegen-units=1" >&2
...
if diff -q "$WORK/out_cxx.txt" "$WORK/out_rust.txt" >/dev/null; then
  echo "PASS: C++ and Rust agree on all $lines probe lines (split-blob walks, boundary sizes, node headers)."
else
