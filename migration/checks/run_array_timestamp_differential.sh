#!/usr/bin/env bash
# Differential check for array_timestamp.cpp -- the only real evidence for this unit.
#
...
# array_timestamp.cpp.o's undefined set reaches Node, Allocator and terminate, so
...
WORK="$ROOT/build/out/array-timestamp-diff"
...
SRC="$ROOT/migration/checks/array_timestamp_differential.cpp"

echo "==> building driver against C++ array_timestamp"
c++ "${CXXFLAGS[@]}" -o "$WORK/driver_cxx" "$SRC" "$REALM_A" "${LIBS[@]}"

echo "==> building driver against Rust array_timestamp (staticlib first)"
...
# Guard. array_timestamp.cpp has no unique file-static and its ZT* are coalesced, so fall
...
repl=$(awk '/ArrayTimestamp6createEv$/{print $1; exit}' "$WORK/syms_rust.txt")
if [ -z "$repl" ]; then echo "FAIL: ArrayTimestamp::create missing from driver_rust." >&2; exit 1; fi
d=$(( 0x$probe > 0x$repl ? 0x$probe - 0x$repl : 0x$repl - 0x$probe ))
if [ "$d" -gt 65536 ]; then
  echo "FAIL: ArrayTimestamp::create is $d bytes from the crate probe; with codegen-units=1" >&2
...
if diff -q "$WORK/out_cxx.txt" "$WORK/out_rust.txt" >/dev/null; then
  echo "PASS: C++ and Rust agree on all $lines probe lines (null encoding, sentinel collisions, six comparators)."
else
